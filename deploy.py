...
if st.session_state.page == "Complement":
    ...
    * G pairs with C.
    """
    st.write(s)
    seq = st.text_area("Enter Sequence for DNA:", placeholder="Type here...", key='complement')  
    if st.button("Submit"):
        if seq:
            st.write(f'Complement Sequence is: **{Complement(seq)}**')
    fasta_file = st.file_uploader("Upload a FASTA or FNA file", type=["fasta", "fna"],key='complement_fasta')
    if fasta_file:
        sequence = parse_fasta(fasta_file)
        if sequence:
            st.write(f'**Sequence is:** ')
            st.write(sequence)
...
if st.session_state.page == "Reverse":
    ...
    seq = st.text_area("Enter Sequence for DNA:", placeholder="Type here...", key='reverse')
    if st.button("Submit"):
        if seq:
            st.write(f'Reverse Sequence is: **{Reverse(seq)}**')
    fasta_file = st.file_uploader("Upload a FASTA or FNA file", type=["fasta", "fna"], key='reverse_fasta')
    if fasta_file:
        sequence = parse_fasta(fasta_file)
        if sequence:
            st.write(f'**Sequence is:** ')
...
if st.session_state.page == 'Reverse_Complement':
    ...
    seq = st.text_area("Enter Sequence for DNA:", placeholder="Type here...", key='reverse_complement')
    if st.button("Submit"):
        if seq:
            st.write(f'Reverse Complement Sequence is: **{Reverse_Complement(seq)}**')
    fasta_file = st.file_uploader("Upload a FASTA or FNA file", type=["fasta", "fna"],key='reverse_complement_fasta')
    if fasta_file:
        sequence = parse_fasta(fasta_file)
        if sequence:
            st.write(f'**Sequence is:**')
            st.write(sequence)
            st.write(f'**Reverse Complement Sequence is:** ')
            st.write(Reverse_Complement(sequence))
...
if st.session_state.page == 'Translation_Table':
    ...
    seq = st.text_area("Enter Sequence for DNA:", placeholder="Type here...", key='translation_table')
    if st.button("Submit"):
        if seq:
            st.write(f'Translation Table of Sequence is: **{Translation_Table(seq)}**') 

    fasta_file = st.file_uploader("Upload a FASTA or FNA file", type=["fasta", "fna"], key='translation_table_fasta')
    if fasta_file:
        sequence = parse_fasta(fasta_file)
        if sequence:
            st.write(f'**Sequence is:** ')
            st.write(sequence)
            st.write(f'**Translation Table of Sequence is:** ')
            st.write(Translation_Table(sequence))
...
if st.session_state.page == 'Match':
    ...
    seq = st.text_area("Enter Sequence for DNA:", placeholder="Type here...", key='match')
    sub_seq = st.text_input("Enter the sub-sequence to search for:", placeholder="Type here...", key='sub_match')
    if st.button("Submit"):
        if seq and sub_seq:
    ...
    fasta_file = st.file_uploader("Upload a FASTA or FNA file", type=["fasta", "fna"], key='match_fasta')
    if fasta_file:
        sequence = parse_fasta(fasta_file)
        if sequence:
            st.write(f'**Sequence is:** ')
            st.write(sequence)
    sub_seq_fasta = st.text_input("Enter the sub-sequence to search in fasta file:", placeholder="Type here...", key='sub_match_fasta')
...
if st.session_state.page == 'Bad':
    ...
    seq = st.text_area("Enter Sequence for DNA:", placeholder="Type here...", key='bad')
    sub_seq = st.text_input("Enter the sub-sequence to search for:", placeholder="Type here...", key='sub_bad')
    if st.button("Submit"):
        if seq and sub_seq:
    ...
    fasta_file = st.file_uploader("Upload a FASTA or FNA file", type=["fasta", "fna"], key='bad_fasta')
    if fasta_file:
        sequence = parse_fasta(fasta_file)
        if sequence:
            st.write(f'**Sequence is:** ')
            st.write(sequence)
    sub_seq_fasta = st.text_input("Enter the sub-sequence to search in fasta file:", placeholder="Type here...", key='sub_bad_fasta')
    if st.button("Submit Fasta"):
        if fasta_file and sub_seq_fasta:
...
if st.session_state.page == 'Index':
    ...
    seq = st.text_area("Enter Sequence for DNA:", placeholder="Type here...", key='index')
    ln = st.number_input("Enter the length of the substring:", min_value=0, max_value=len(seq), step=1, key='len_index')
    ln = int(ln)
    if st.button("Submit"):
        if seq and ln:
    ...
    fasta_file = st.file_uploader("Upload a FASTA or FNA file", type=["fasta", "fna"], key='index_fasta')
    sequence = ''
    if fasta_file:
        sequence = parse_fasta(fasta_file)
        if sequence:
            st.write(f'**Sequence is:**')
            st.write(sequence)
    ln_fasta = st.number_input("Enter the length of the substring to search in fasta file:", min_value=0, max_value=len(sequence), step=1, key='len_index_fasta')
...
if st.session_state.page == 'Query':
    ...
    seq = st.text_area("Enter Sequence for DNA:", placeholder="Type here...", key='query')
    sub_seq = st.text_input("Enter the sub-sequence to search for:", placeholder="Type here...", key='sub_query')
    if st.button("Submit"):
        if seq and sub_seq:
    ...
    fasta_file = st.file_uploader("Upload a FASTA or FNA file", type=["fasta", "fna"], key='query_fasta')
    if fasta_file:
        sequence = parse_fasta(fasta_file)
        if sequence:
            st.write(f'**Sequence is:**')
            st.write(sequence)
    sub_seq_fasta = st.text_input("Enter the sub-sequence to search in fasta file:", placeholder="Type here...", key='sub_query_fasta')
    if st.button("Submit Fasta"):
        if fasta_file and sub_seq_fasta:
...
if st.session_state.page == 'Suffix_array_construction':
    ...
    seq = st.text_area("Enter Sequence for DNA:", placeholder="Type here...", key='suffix')
    if st.button("Submit"):
        if seq:
    ...
    fasta_file = st.file_uploader("Upload a FASTA or FNA file", type=["fasta", "fna"], key='suffix_fasta')
    if fasta_file:
        sequence = parse_fasta(fasta_file)
        if sequence:
...
if st.session_state.page == 'Overlap':
    ...
    seq1 = st.text_area("Enter Sequence for DNA:", placeholder="Type here...", key='overlap1')
    seq2 = st.text_area("Enter Sequence for DNA to Overlap with other Sequence:", placeholder="Type here...", key='operlap2')
    if st.button("Submit"):
        if seq1 and seq2:
...
if st.session_state.page == 'native_overlap':
    ...
    seq = st.text_area("Enter each DNA Sequence using the separator  ',' :", placeholder="Type here...",key='native')
    k = st.number_input("Enter the minimum length of the overlap:", min_value=0, step=1, key='minimum_k')
    k = int(k)
    if st.button("Submit"):
        if seq and k:
